    ...
    <div className="bg-page d-flex flex-row align-items-center dark:bg-transparent">
      <Container className='mw-100'>
				...
				<Form.Group className="p-5 rounded-5 bg-group">
					...
					<Row>
						<Col>
							<Dropdown onSelect={onSelectVestingId}>
								<Dropdown.Toggle className="btn-lg bg-edited text-black-50 w-100 border-0" disabled={!VESTING_IDS || VESTING_IDS.length == 0 || !CAN_TYPE}>
									{ X_VESTING_ID }
								</Dropdown.Toggle>
					...
					<Row>
						<Col><Button type="submit" className="d-flex justify-content-center w-100 btn-lg bg-button p-2 fw-bold border-0" disabled={!CAN_TYPE || !X_VESTING_ID} onClick={() => deleteVesting()}> {KEY_ICON()}Delete</Button></Col>
						<Col><Button type="submit" className="d-flex justify-content-center w-100 btn-lg bg-button p-2 fw-bold border-0" disabled={!CAN_TYPE || !X_VESTING_CLIFF_DAYS || !X_VESTING_DURATION_DAYS || !X_VESTING_NUM_SLIDES || !X_VESTING_START_MILLIS} onClick={() => saveVesting()}> {KEY_ICON()}Save</Button></Col>
						<Col><Button type="submit" className="d-flex justify-content-center w-100 btn-lg bg-button p-2 fw-bold border-0" disabled={!CAN_TYPE || (!X_VESTING_CLIFF_DAYS && !X_VESTING_DURATION_DAYS && !X_VESTING_NUM_SLIDES && !X_VESTING_START_MILLIS)} onClick={() => cancelVesting()}> {KEY_ICON()}Cancel</Button></Col>
					</Row>
